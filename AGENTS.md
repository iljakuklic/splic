...
### Clippy
The project enforces a curated set of lints beyond Clippy defaults — see `just clippy` in the justfile for the full list. All lints are `-D` (hard errors). Use `#[expect(...)]` to suppress a lint at a specific site. For test modules it is acceptable to use a broad `#![allow(...)]` at the top of the file rather than per-site annotations.
