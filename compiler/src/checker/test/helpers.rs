...
pub fn test_ctx(arena: &bumpalo::Bump) -> Ctx<'_, '_, '_> {
    static EMPTY: std::sync::OnceLock<
        HashMap<&'static Name, &'static core::Term<'static, 'static>>,
...
pub fn test_ctx_with_globals<'names, 'core, 'globals>(
    ...
    globals: &'globals HashMap<&'names Name, &'core core::Term<'names, 'core>>,
) -> Ctx<'names, 'core, 'globals> {
    Ctx::new(arena, globals)
}

/// Helper: build a Pi term for a function `fn f() -> u64` (no params, meta phase).
pub fn sig_no_params_returns_u64(arena: &bumpalo::Bump) -> &core::Term<'_, '_> {
...
pub fn sig_one_param_returns_u64(arena: &bumpalo::Bump) -> &core::Term<'_, '_> {
    let params =
